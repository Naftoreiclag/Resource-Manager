...
void convertWaveform(const boost::filesystem::path& fromFile, const boost::filesystem::path& outputFile, const Json::Value& params, bool modifyFilename) {
    ...
    errorCode = ov_fopen(fromFile.string().c_str(), &ovFile);
    
    if(errorCode) {
        std::cout << "\tFailed to read waveform!" << std::endl;
        return;
    }
    const vorbis_info* inputVorbisInfo = ov_info(&ovFile, -1);
    ...
    while(true) {
        int ovRet = ov_read(&ovFile, inputBuffer, sizeof(inputBuffer), bigEndian, wordSize, signage, &logicalBitstreamPtr);
        ...
        // Positive return values are the number of bytes read
            ...
            for(int i = 0; i < numSamplesRead; ++ i) {
                ...
                for(int j = 0; j < wordSize; ++ j) {
                    sampleData = sampleData | ((inputBuffer[(i * wordSize) + j] & 0xff) << (j * 8));
                }
                */
